...
class PositionController extends DefaultController
{
    ...
    public function __construct()
    {
        ...
        $this->tableHeadersWithCb = [
            ...
            ['name' => 'Event', 'column' => 'event_name', 'order' => true],
            ['name' => 'Department', 'column' => 'department', 'order' => true],
            ['name' => 'Created at', 'column' => 'created_at', 'order' => true],
            ['name' => 'Updated at', 'column' => 'updated_at', 'order' => true],
        ];
    ...
    protected function defaultDataQuery()
    {
        ...
        $dataQueries = Position::join('departments', 'departments.id', '=', 'positions.department_id')
        ->where($filters)
        ->where(function ($query) use ($orThose) {
            $query->where('positions.name', 'LIKE', '%' . $orThose . '%')
                ->orWhere('departments.name', 'LIKE', '%' . $orThose . '%');
        })
        ->orderBy($orderBy, $orderState)
        ->select('positions.*', 'departments.name as department');
