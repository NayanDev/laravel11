    ...
    <div class="pc-content">
        ...
        <div class="page-block">
            ...
            <div class="col-md-12">
                ...
                <a class="btn btn-secondary float-end text-white mx-1" data-bs-toggle="modal" onclick="openBulkModal()" data-bs-target="#createBulkForm-{{$uri_key}}">
                    Bulk Create
                </a>
                @endif
                <!-- <ul class="breadcrumb">
                ...
                <div class="card mb-4">
                    <div class="card-body p-3">
                        ...
                        <div class="table-responsive p-0">
                            <table id="table-list-{{$uri_key}}" class="table table-hover" numbering="false">
                                <thead>
                                    <tr>
                                        <th><input type="checkbox" class="cb-all-{{$uri_key}}"></th>
...
<div class="modal fade" tabindex="-1" role="dialog" id="createBulkForm-{{$uri_key}}">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id='form-bulk-create-{{$uri_key}}' action="{{url('question-bulk-create')}}" method="post">
                    {{ csrf_field() }}
                    <p class="text-bulk"></p>
                    ...
                    <input type="hidden" id="txt-cb-{{$uri_key}}" name="question_ids" value="[]">
                    <hr>
                    <button id="btn-for-form-bulk-create-{{$uri_key}}" type="button" class="btn btn-outline-secondary" onclick="softSubmit('form-bulk-create-{{$uri_key}}', 'list-{{$uri_key}}')">Submit</button>
                </form>
            </div>
        </div>
    </div>
...
@endif
<script>
    var arrSavedInstance = []
    var paramCb = {
            classAllCb : ".cb-all-{{$uri_key}}",
            classSingleCb : ".cb-{{$uri_key}}",
            inputHidden : "#txt-cb-{{$uri_key}}",
        }
    ...
    $(document).ajaxStop(function() {
        checkboxAction(arrSavedInstance, paramCb)
    });
    ...
    {
        var realCounting = JSON.parse($(paramCb.inputHidden).val())
        $(".text-bulk").html("Anda memilih <b>"+arrSavedInstance.length+"</b> Item")
    }
